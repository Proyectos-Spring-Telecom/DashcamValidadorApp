...
import '../services/meter_fare_service.dart';
import '../services/wallet_service.dart';
import '../config/app_config.dart';
import '../services/global_gps_service.dart';
import '../utils/logger.dart';

class MeterFareScreen extends StatefulWidget {
  const MeterFareScreen({super.key});

  @override
  State<MeterFareScreen> createState() => _MeterFareScreenState();
}

class _MeterFareScreenState extends State<MeterFareScreen> {
  ...
  void initState() {
    ...
    _ensureGpsActive();
    // Actualizar la UI cada segundo para mostrar el tiempo transcurrido y distancia acumulada
  ...
  Future<void> _processCard(String uid) async {
    ...
    if (meterFareService.hasActiveTrip(uid)) {
      // Finalizar el viaje
      final completedTrip = await meterFareService.endTrip(uid);
      if (!mounted) return;

      if (completedTrip != null) {
        setState(() {
          _statusMessage = '✅ Viaje finalizado\nDistancia recorrida: ${completedTrip.distance!.toStringAsFixed(2)}m\nTarifa: \$${completedTrip.fare!.toStringAsFixed(2)}';
          _isProcessing = false;
        });
      ...
      final started = await meterFareService.startTrip(uid);
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final activeTrips = meterFareService.activeTrips;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Tarifa por metro'),
        leading: IconButton(
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          meterFareService.cancelTrip(monedero);
                                        },
