...
import '../models/meter_fare_trip.dart';
import '../services/global_gps_service.dart';
import '../services/native_gps_service.dart';
import '../services/wallet_service.dart';
import '../utils/logger.dart';

/// Servicio para manejar los viajes de tarifa por metro
/// Calcula la distancia total recorrida durante el trayecto usando actualizaciones GPS en tiempo real
...
class MeterFareService extends ChangeNotifier {
  // Mapa de viajes activos por número de serie del monedero
  final Map<String, MeterFareTrip> _activeTrips = {};
  ...
  Map<String, MeterFareTrip> get activeTrips => Map.unmodifiable(_activeTrips);
  ...
  Future<bool> startTrip(String numeroSerieMonedero) async {
    ...
    // Obtener coordenadas GPS iniciales con timeout
    double lat = 0.0;
    double lon = 0.0;
  ...
  /// Usamos addListener porque globalGpsService ya tiene la suscripción activa al stream nativo
  ...
  Future<MeterFareTrip?> endTrip(String numeroSerieMonedero) async {
    ...
    // No cancelar la suscripción GPS aquí, ya que puede haber otros viajes activos
    // La suscripción se cancelará automáticamente cuando no haya más viajes activos
  ...
  MeterFareTrip? getActiveTrip(String numeroSerieMonedero) {
  ...
  // NOTA: Este servicio es un singleton y NO debe ser destruido durante la vida de la app
  // El método dispose() NO se debe llamar nunca, ya que el servicio persiste entre navegaciones
  // Solo se limpia cuando la app se cierra completamente
  // Si necesitas limpiar viajes, usa clearAllTrips() explícitamente
}

// Instancia singleton del servicio - persiste durante toda la vida de la app
final meterFareService = MeterFareService();
